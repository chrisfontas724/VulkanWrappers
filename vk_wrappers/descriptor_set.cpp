...
void DescriptorSet::set_buffer(uint32_t index, std::shared_ptr<ComputeBuffer> buffer) const {
    ...
    vk::WriteDescriptorSet write_info(descriptor_set_, index, 0, 1, buffer->type(), nullptr,
                                      &buffer_info);
...
void DescriptorSet::set_texture(uint32_t index, const ComputeTexturePtr& texture) const {
    ...
    vk::WriteDescriptorSet write_info(descriptor_set_, index, 0, 1, texture->type(), &image_info);
    device->vk().updateDescriptorSets(1, &write_info, 0, nullptr);
}
