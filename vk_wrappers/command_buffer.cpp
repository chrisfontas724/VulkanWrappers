...
static const State kDefaultState {
 ...
 std::shared_ptr<CommandBuffer> CommandBuffer::create(std::shared_ptr<LogicalDevice> device, Queue::Type queue, vk::CommandBufferLevel level) {
...
void CommandBuffer::beginRenderPass(const vk::RenderPass& render_pass, const FrameBuffer& framebuffer,
    ...
    vk::RenderPassBeginInfo info(render_pass,
                                 framebuffer.vk(),
...
void CommandBuffer::dispatchBase(uint32_t base_x, uint32_t base_y, uint32_t base_z, uint32_t group_x, uint32_t group_y, uint32_t group_z) {
    CXL_DCHECK(group_x > 0 && group_y > 0 && group_z > 0) << group_x << " " << group_y << " " << group_z;
    command_buffer_.dispatchBase(base_x, base_y, base_z, group_x, group_y, group_z);
}

void CommandBuffer::copyBufferToImage(std::shared_ptr<gfx::ComputeBuffer> buffer, vk::Image image, uint32_t width, uint32_t height) {
    ...
    command_buffer_.copyBufferToImage(buffer->vk(), image, vk::ImageLayout::eTransferDstOptimal, 1, &region_info);
}
...
void CommandBuffer::copyImageToBuffer(ComputeTexturePtr texture, ComputeBufferPtr buffer) {
    ...
    vk::BufferImageCopy region_info(0,0,0, layers, offset, extent);
    command_buffer_.copyImageToBuffer(texture->image(), texture->layout(), buffer->vk(), 1, &region_info);
...
void CommandBuffer::copyImageToImage(ComputeTexturePtr src, ComputeTexturePtr dst) {
    ...
    command_buffer_.copyImage(src->image(), src->layout(), dst->image(), dst->layout(), 1, &copy_info);
 ...
 void CommandBuffer::clearColorImage(ComputeTexturePtr image, std::array<float,4> color) {
    ...
    command_buffer_.clearColorImage(image->image(), image->layout(), &clear_color, 1, &resource_range);
    image->transitionImageLayout(*this, layout);
 }


void CommandBuffer::transitionImageLayout(vk::Image& image, vk::Format, vk::ImageLayout old_layout, vk::ImageLayout new_layout) {
    vk::ImageSubresourceRange resource_range(vk::ImageAspectFlagBits::eColor, 0,1,0,1);
    ...
    switch(old_layout) {
        ...
        default:
            throw std::invalid_argument("Unsupported source layout transition!" + vk::to_string(old_layout));
    }
    ...
    switch(new_layout) {
        ...
        default:
            throw std::invalid_argument("Unsupported destination layout transition!" + vk::to_string(new_layout));

    }
    command_buffer_.pipelineBarrier(source_stage, destination_stage, vk::DependencyFlags(), 0, nullptr, 0, nullptr, 1, &barrier);
}

void CommandBuffer::draw(uint32_t num_vertices, uint32_t instance_count, uint32_t first_vertex, uint32_t first_instance) {
    auto pipeline = state_map[identifier_].pipeline.lock();
...
void CommandBuffer::blit(ComputeTexturePtr src, ComputeTexturePtr dst, vk::Filter filter) {
    ...
    command_buffer_.blitImage(src->image(), src->layout(), 
                              dst->image(), dst->layout(),
                              1, &blit_info, filter);
}

bool CommandBuffer::has_recording() const {
    return state_map[identifier_].has_recording;
}
