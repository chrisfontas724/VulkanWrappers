...
#ifndef DALI_GRAPHICS_VK_WRAPPERS_COMPUTE_TEXTURE_HPP_
#define DALI_GRAPHICS_VK_WRAPPERS_COMPUTE_TEXTURE_HPP_
...
public:
    ...
    ComputeTexture(std::shared_ptr<LogicalDevice> device, uint32_t width, uint32_t height, uint32_t channels, const uint8_t* pixels, bool lerp = true, bool unorm = false);
    ComputeTexture(std::shared_ptr<LogicalDevice> device, uint32_t width, uint32_t height, const half* pixels, bool lerp = true, bool unorm = false);
    ComputeTexture(std::shared_ptr<LogicalDevice> device, const ImageUtils::Data& data);
    ComputeTexture(vk::ImageView view, 
                   vk::Image image,
...
#endif // DALI_GRAPHICS_VK_WRAPPERS_COMPUTE_TEXTURE_HPP_