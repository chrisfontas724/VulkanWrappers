...
LogicalDevice::LogicalDevice(const std::shared_ptr<PhysicalDevice>& physical_device,
    ...
    try {
        ...
        queue_families_map_ = {{Queue::Type::kGraphics, indices.graphics_family.value()},
        ...
        vk::DeviceCreateInfo info({}, queue_create_infos.size(), queue_create_infos.data(), 0,
                                  nullptr, extensions.size(), extensions.data());
        info.pNext = &features;
        device_ = physical_device->vk().createDeviceUnique(info);

        // Make queues.
        vk::Queue present_queue =
            device_->getQueue(queue_family_index(gfx::Queue::Type::kPresent), 0);
        vk::Queue graphics_queue =
            device_->getQueue(queue_family_index(gfx::Queue::Type::kGraphics), 0);
        vk::Queue compute_queue =
            device_->getQueue(queue_family_index(gfx::Queue::Type::kCompute), 0);
        vk::Queue transfer_queue =
            device_->getQueue(queue_family_index(gfx::Queue::Type::kTransfer), 0);
...
std::vector<vk::Semaphore> LogicalDevice::createSemaphores(
    const uint32_t& num, const vk::SemaphoreCreateFlags& flags) const {
...
std::vector<vk::Fence> LogicalDevice::createFences(const uint32_t& num,
...
const Queue& LogicalDevice::getQueue(Queue::Type type) const { return *queues_.at(type).get(); }

void LogicalDevice::waitIdle() const { device_->waitIdle(); }
