...
PhysicalDevice::PhysicalDevice(vk::PhysicalDevice physical_device)
    ...
    features_ = physical_device_.getFeatures2();

    // Requesting ray tracing properties

    properties_.pNext = &ray_trace_properties_;
    physical_device_.getProperties2(&properties_);
   ...
   // ray_trace_properties_  = properties_chain.get< vk::PhysicalDeviceRayTracingPipelinePropertiesKHR>();
    CXL_VLOG(2) << "MAX RECURSION DEPTH: " << ray_trace_properties_.maxRayRecursionDepth;
}
...
void PhysicalDevice::printDiagnostics() {
    ...
    CXL_LOG(INFO) << "        a. Has Geometry Shaders: "
                  << (features_.features.geometryShader ? "yes" : "no");
    CXL_LOG(INFO) << "\n\n";
}
...
uint32_t PhysicalDevice::performanceScore(const vk::SurfaceKHR& surface,
    ...
    uint32_t final_score = (indices.isComplete() && extensions_supported && swap_chain_adequate &&
                            features_.features.samplerAnisotropy)
                               ? score
                               : 0;
